            ...
            if local_id in remote_plugin_map:
                ...
                try:
                    ...
                    if remote_version > local_version:
                        self.app.write_log(f"[Update] New version available for {local_id}: {remote_version} (current: {local_version})")
                        local_plugin['update_available'] = True
                        local_plugin['latest_version'] = remote_version_str
                except packaging.version.InvalidVersion:
                    self.app.write_warn(f"[Update] Could not compare versions for {local_id} ('{local_version_str}' vs '{remote_version_str}')")

    def update_plugin(self, plugin_id):
        self.app.write_log(f"[Update] Starting update for plugin: {plugin_id}")
        
        remote_plugin_data = next((p for p in self.app.remote_plugins if p['id'] == plugin_id), None)
        ...
        try:
            ...
            if path_to_delete.is_dir(): shutil.rmtree(path_to_delete)
            else: path_to_delete.unlink()
        ...
        if not self.install_from_url(remote_plugin_data, reload_plugins=False):
            self.app.write_error(f"[Update] Failed to install new version of {plugin_id}. The plugin has been uninstalled.")
            return False
